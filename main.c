...
Triangle(float x, float z, float xx, float zz)
{
...
Wall(int n)
{
    ...
    for (int i = 1; i < n + 1; i++)
    {
        ...
        x = 2 * ((cos(((PI / 3) + (2 * PI * i)) / n)));
        z = 2 * ((sin(((PI / 3) + (2 * PI * i)) / n)));
        zz = 2 * ((sin(((PI / 3) + (2 * PI * (i + 1))) / n)));
        xx = 2 * ((cos(((PI / 3) + (2 * PI * (i + 1))) / n)));
        Triangle(x, z, xx, zz);
    }
...
Polygon(int n)
{
    ...
    y = 0;
    glBegin(GL_TRIANGLE_FAN);
    glMaterialfv(GL_FRONT, GL_AMBIENT_AND_DIFFUSE, materials[5]);
    for (int i = 1; i < n + 1; i++)
    {
        x = 2 * ((cos(((PI / 3) + (2 * PI * i)) / n)));
        z = 2 * ((sin(((PI / 3) + (2 * PI * i)) / n)));
