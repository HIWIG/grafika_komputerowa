...
public class Lab7 extends JPanel implements GLEventListener {
	...
	private Texture textureFromResource(String resourceName) {
		...
		try {
			displayGL.getContext().makeCurrent();
			GL2 ctx = displayGL.getContext().getGL().getGL2();
			TextureData texData = TextureIO.newTextureData(ctx.getGLProfile(), new File(resourceName), true, null);
			return TextureIO.newTexture(ctx, texData);
